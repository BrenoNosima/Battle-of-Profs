<!--
  phaser-vue-gravity.vue
  
  Versão com gravidade e sem pulo:
  - Canvas em tela cheia
  - Gravidade e física de plataforma
  - Sem função de pulo (W)
  - Movimento apenas lateral (A e D)
-->
    ...
    initGame() {
      ...
      class FightScene extends Phaser.Scene {
        ...
        create() {
          ...
          // Criar o jogador
          this.player = this.physics.add.sprite(this.cameras.main.width * 0.25, this.cameras.main.height / 2, 'player');
          ...
          // Criar o inimigo
          this.enemy = this.physics.add.sprite(this.cameras.main.width * 0.75, this.cameras.main.height / 2, 'enemy');
        ...
        createPlatforms() {
          ...
          // Plataforma principal (solo)
          const groundY = this.cameras.main.height - 50;
          const ground = this.platforms.create(this.cameras.main.width / 2, groundY, 'platform');
          ground.setScale(this.cameras.main.width / ground.width, 1).refreshBody();
          
          // Adicionar algumas plataformas flutuantes
          const platformWidth = 200;
          const platformHeight = 20;
          
          // Plataforma à esquerda
          const platform1 = this.platforms.create(
            this.cameras.main.width * 0.25, 
            groundY - 150, 
            'platform'
          );
          platform1.setScale(platformWidth / platform1.width, platformHeight / platform1.height).refreshBody();
          
          // Plataforma à direita
          const platform2 = this.platforms.create(
            this.cameras.main.width * 0.75, 
            groundY - 150, 
            'platform'
          );
          platform2.setScale(platformWidth / platform2.width, platformHeight / platform2.height).refreshBody();
          
          // Plataforma central mais alta
          const platform3 = this.platforms.create(
            this.cameras.main.width * 0.5, 
            groundY - 250, 
            'platform'
          );
          platform3.setScale(platformWidth / platform3.width, platformHeight / platform3.height).refreshBody();
        }
